...
if np.isnan(args.avg_hrs):
    ...
    print("`--avg-hrs` is set to `--skip-hrs` : %d hrs" % (args.skip_hrs,))
...
def work(dt, output_filename):
    ...
    try: 
        ...
        data_snp1 = dlh.loadDataByDate(dt,           msm, **crop_kwargs, datasets=["diag_snaps_state",])
        data_snp2 = dlh.loadDataByDate(dt + avg_hrs, msm, **crop_kwargs, datasets=["diag_snaps_state",])
        ...
        for varnameA, varnameB in [
            ...
            ("G_mas",   "TEND_MASFLX"),
            ("G_nsw",   "TEND_SFCFLX"),
            ("G_mflx",  "TEND_SFC_WTHMASS"),
            ("G_res",   "TEND_RES"),
            ("DIFF_oceQnet",   "DIFF_oceQnet"),
        ]:
            print("Var: ", varnameA)
        ...
        data["G_adv_mflx"] = data["G_adv"] + data["G_mflx"]
       
        data["G_hdiff"] = data["G_diffx"] + data["G_diffy"]
...
with Pool(processes=args.nproc) as pool:
    ...
    for i, dt in enumerate(dts):
        ...
        dtstr = dt.strftime("%Y-%m-%d_%H")
        output_filename = "%s/heat_budget_analysis_%s.nc" % (args.output_dir, dts[i].strftime("%Y-%m-%d_%H"))
